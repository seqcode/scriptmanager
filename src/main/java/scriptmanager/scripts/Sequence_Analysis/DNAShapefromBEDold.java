 ...
 */
public class DNAShapefromBEDold {
	...
	public DNAShapefromBEDold(File gen, File b, File out, boolean[] type, boolean str, boolean outputComposite, short outputMatrix, boolean gzOutput) {
		GENOME = gen;
		BED = b;
		OUTBASENAME = out;
		OUTPUT_TYPE = type;
	 ...
	 */
	public DNAShapefromBEDold(File gen, File b, File out, boolean[] type, boolean str, boolean outputComposite, short outputMatrix, boolean gzOutput, PrintStream[] ps) {
		GENOME = gen;
		BED = b;
		OUTBASENAME = out;
		OUTPUT_TYPE = type;
					...
					if (!seq.contains("N")) {
						...
						if (OUTPUT_TYPE[0]) {
							if (y == 0) {
								...
								if (OUTPUT_MATRIX != DNAShapefromBEDold.NO_MATRIX) {
									// print header
									OUT_M.print("YORF");
									if (OUTPUT_MATRIX == DNAShapefromBEDold.CDT) {
										OUT_M.print("\tNAME");
									}
									// print domain
	...
	private void openOutputFiles() throws FileNotFoundException, IOException {
		...
		// Open Output File
		if (OUTPUT_MATRIX > 0) {
			String SUFFIX = (OUTPUT_MATRIX==DNAShapefromBEDold.CDT ? ".cdt" : ".tab") + (GZIP_OUTPUT? ".gz": "");
			if (OUTPUT_TYPE[0]) {
